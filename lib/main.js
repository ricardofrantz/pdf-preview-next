...
(function () {
  ...
  function setHash(hash) {
    ...
    try {
      ...
      console.warn('PDF Viewer: failed to apply document hash', err)
    }
  }
  window.addEventListener('load', async function () {
    const config = loadConfig()
    const defaults = config.defaults
    const documentHash = hashFromUrl(config.path)
    ...
    PDFViewerApplication.initializedPromise.then(() => {
      ...
      PDFViewerApplication.open(config.path, loadOpts).then(function () {
        if (PDFViewerApplication.pdfDocument) {
          PDFViewerApplication.pdfDocument._pdfInfo.fingerprints = [config.path]
        }
      }).catch(function (err) {
