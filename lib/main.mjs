...
class PdfPreviewApp {
  constructor() {
    ...
    this.thumbnailRendering = new Set();
  ...
  async loadDocument({
    ...
    try {
      ...
      if (userInitiated && previousActiveElement) {
  ...
  queueThumbnailRender(pageNumber) {
    if (
      !this.pdfDocument ||
      this.thumbnailCanvases.has(pageNumber) ||
      this.thumbnailRendering.has(pageNumber)
    ) {
      return;
    }
    ...
    this.thumbnailRendering.add(pageNumber);
    void this.renderThumbnail(pageNumber, item, token)
      .catch((error) => {
        console.warn('PDF Preview: failed to render thumbnail', error);
      })
      .finally(() => {
        this.thumbnailRendering.delete(pageNumber);
      });
  }

  async renderThumbnail(pageNumber, item, token) {
    ...
    const shell = item.querySelector('.thumbnail-canvas-shell');
    if (!shell) {
      return;
    }
    shell.replaceChildren(canvas);
    this.thumbnailCanvases.set(pageNumber, {
      item,
  ...
  enforceThumbnailCanvasLimit() {
    if (this.thumbnailCanvases.size <= THUMBNAIL_MAX_CANVASES) {
      return;
    }
    const activePage = this.pdfViewer.currentPageNumber || 1;
    const candidates = Array.from(this.thumbnailCanvases.entries()).sort(
      ([leftPage, left], [rightPage, right]) => {
        const leftVisible = this.visibleThumbnailPages.has(leftPage) ? 1 : 0;
        const rightVisible = this.visibleThumbnailPages.has(rightPage) ? 1 : 0;
        if (leftPage === activePage) {
          return 1;
        }
        if (rightPage === activePage) {
          return -1;
        }
        return leftVisible - rightVisible || left.lastUsed - right.lastUsed;
      },
    );
  ...
  updateActiveThumbnail() {
    ...
    this.queueThumbnailRender(activePage);
  }
