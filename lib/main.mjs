...
class PdfPreviewApp {
  ...
  async loadDocument({ restoreView = false } = {}) {
    ...
    try {
      ...
      await pagesInit;
      await this.populateOutline(pdfDocument, token);
  ...
  applyDocumentView(viewState) {
    ...
    const initialViewState =
      viewState || (documentHash ? null : this.config.initialViewState);
  ...
  async populateOutline(pdfDocument, token) {
    let outline = null;
    try {
    ...
    this.setOutlineVisible(this.config.defaults?.sidebar === true);
  }
